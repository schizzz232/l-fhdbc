...
class TestSearxSearch(unittest.TestCase):
    ...
    def setUp(self):
        os.environ["SEARXNG_BASE_URL"] = (
            "http://127.0.0.1:8080"  # Set the environment variable
        )
    ...
    def test_execute_valid_query(self):
        ...
        self.assertNotEqual(
            result, "", "Result should not be empty. Check SearxNG instance."
        )
    ...
    def test_execute_request_exception(self):
        ...
        try:
            ...
            self.search_tool.base_url = (
                original_base_url  # Restore the original base_url
            )
    ...
    def test_execute_no_results(self):
        ...
        if result == "":
            print(
                "Warning: SearxNG returned no results for a query that should have returned no results."
            )
