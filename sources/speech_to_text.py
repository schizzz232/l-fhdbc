        ...
        while not done:
            ...
            for _ in range(0, int(self.rate / self.chunk * self.record_seconds)):
                try:
                    ...
                    print(
                        Fore.RED
                        + f"AudioRecorder: Failed to read stream - {e}"
                        + Fore.RESET
                    )
            ...
            if self.verbose:
                print(
                    Fore.GREEN
                    + "AudioRecorder: Added audio chunk to queue"
                    + Fore.RESET
                )
    ...
    def _transcribe(self) -> None:
        ...
        while not done or not audio_queue.empty():
            try:
                ...
                print(
                    Fore.YELLOW
                    + f"Transcribed: {text} in {end_time - start_time} seconds"
                    + Fore.RESET
                )
                ...
                for language, words in self.confirmation_words.items():
                    if any(word in text.lower() for word in words):
                        print(
                            Fore.GREEN
                            + f"Trigger detected. Sending to AI..."
                            + Fore.RESET
                        )
