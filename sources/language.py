...
class LanguageUtility:
    ...
    def load_model(self) -> None:
        ...
        self.translators_model = {
            lang: MarianMTModel.from_pretrained(f"Helsinki-NLP/opus-mt-{lang}-en")
            for lang in self.supported_language
            if lang != "en"
        }
    ...
    def translate(self, text: str, origin_lang: str) -> str:
        ...
        if origin_lang not in self.translators_tokenizer:
            pretty_print(
                f"Language {origin_lang} not supported for translation", color="error"
            )
            return text
    ...
    def analyze(self, text):
        ...
        try:
            ...
            return {"language": language, "emotions": emotions}
        except Exception as e:
            raise e
...
if __name__ == "__main__":
    ...
    test_texts = ["I am so happy today!", "我不要去巴黎", "La vie c'est cool"]
    for text in test_texts:
        ...
        pretty_print(
            f"Translation: {trans} - from: {result['language']} - Emotion: {result['emotions']}"
        )
