        ...
        for block in blocks:
            ...
            if "plan" in line_json:
                for task in line_json["plan"]:
                    if task["agent"].lower() not in [
                        ag_name.lower() for ag_name in self.agents.keys()
                    ]:
                        self.logger.warning(f"Agent {task['agent']} does not exist.")
                        pretty_print(
                            f"Agent {task['agent']} does not exist.", color="warning"
                        )
                        return []
                    agent = {
                        "agent": task["agent"],
                    ...
                    self.logger.info(
                        f"Created agent {task['agent']} with task: {task['task']}"
                    )
                    if "need" in task:
                        self.logger.info(
                            f"Agent {task['agent']} was given info:\n {task['need']}"
                        )
