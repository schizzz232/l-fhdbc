...
from sources.utility import pretty_print, timer_decorator
...
class Memory:
    ...
    def push(self, role: str, content: str) -> int:
        ...
        if self.memory[curr_idx - 1]["content"] == content:
            pretty_print(
                "Warning: same message have been pushed twice to memory", color="error"
            )
    ...
    def summarize(self, text: str, min_length: int = 64) -> str:
        ...
        self.logger.info(
            f"Memory summarization success from len {len(text)} to {len(summary)}."
        )
        return summary
